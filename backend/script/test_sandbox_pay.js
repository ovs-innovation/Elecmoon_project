...
async function testSandboxPay() {
  const merchantId = process.env.PHONEPE_MERCHANT_ID;
  const saltKey = process.env.PHONEPE_SALT_KEY;
  const saltIndex = process.env.PHONEPE_SALT_INDEX || "1";
  const hostUrl = process.env.PHONEPE_HOST_URL;

  console.log(`Testing PhonePe Sandbox API (${hostUrl}) with Merchant ID: ${merchantId}`);
  ...
  try {
    ...
    console.log("SUCCESS! PhonePe Sandbox Response:", JSON.stringify(res.data, null, 2));
    if (res.data?.data?.instrumentResponse?.redirectInfo?.url) {
      console.log("\n>>> REDIRECT URL TO OPEN IN BROWSER:\n", res.data.data.instrumentResponse.redirectInfo.url);
    }
  } catch (err) {
    if (err.response) {
      console.log("PhonePe Sandbox Response Error Status:", err.response.status, JSON.stringify(err.response.data, null, 2));
    } else {
